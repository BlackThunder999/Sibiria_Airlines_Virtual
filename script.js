const NobuChirp = (() => {
    ...
    const ADMIN_PASSWORD = 'NobuChrip2024';
    ...
    const setupComposer = () => {
        ...
        postBtn.addEventListener('click', async () => {
            ...
            postBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Отправка...';
            ...
            postBtn.innerHTML = '<i class="fa-solid fa-feather"></i> Чирикнуть';
            updatePostBtn();
        });
    };
    ...
    const loadFeed = async () => {
        ...
        if (activeTab === 'following') {
            ...
            if (ids.length === 0) { container.innerHTML = '<div style="text-align:center;padding:40px;color:var(--text-secondary)">Подпишитесь на кого-нибудь</div>'; return; }
    ...
    const showComments = async (chirpId) => { const { data } = await supabase.from('comments').select('*').eq('chirp_id', chirpId).order('created_at', { ascending: true }); const overlay = document.createElement('div'); overlay.className = 'modal-overlay'; overlay.innerHTML = `<div class="modal-card"><h3>💬 Комментарии</h3><div style="max-height:50vh;overflow-y:auto;margin-bottom:12px" id="commentsList">${data?.map(c => `<div style="padding:8px 0;border-bottom:1px solid var(--border)"><strong>${esc(c.username)}</strong>: ${esc(c.content)}</div>`).join('') || '<p style="color:var(--text-secondary)">Нет комментариев</p>'}</div><input type="text" id="commentInput" class="modal-input" placeholder="Ваш комментарий..."><button class="modal-btn" id="sendCommentBtn">Отправить</button><button class="modal-btn secondary" id="closeCommentsBtn">Закрыть</button></div>`; document.body.appendChild(overlay); const cc = supabase.channel(`comments-${chirpId}`).on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'comments', filter: `chirp_id=eq.${chirpId}` }, () => { loadCommentsInModal(chirpId); }).subscribe(); document.getElementById('closeCommentsBtn').addEventListener('click', () => { supabase.removeChannel(cc); overlay.remove(); }); document.getElementById('sendCommentBtn').addEventListener('click', async () => { const content = document.getElementById('commentInput').value.trim(); if (!content) return; await supabase.from('comments').insert({ chirp_id: chirpId, user_id: currentUser.id, username: currentUser.username, content }); document.getElementById('commentInput').value = ''; }); };
