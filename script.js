const NobuChirp = (() => {
    ...
    // ========== МГНОВЕННАЯ ПРОВЕРКА БАНА И ПРЕДУПРЕЖДЕНИЙ ==========
    ...
    // ========== ЭКРАН БАНА (без таймера, просто сообщение) ==========
    ...
    const checkWarningsAndBan = async (user) => {
        if (user.is_banned) {
            ...
            else { await supabase.from('users').update({ is_banned: false, ban_reason: null, ban_expires: null }).eq('id', user.id); user.is_banned = false; }
        }
        const { data: warnings } = await supabase.from('warnings').select('*').eq('user_id', user.id).eq('is_read', false);
    ...
    const renderApp = () => {
        ...
        document.getElementById('logoutBtn').addEventListener('click', () => { if (realtimeChannel) supabase.removeChannel(realtimeChannel); if (banCheckInterval) clearInterval(banCheckInterval); localStorage.removeItem('nobu_user'); location.reload(); });
    ...
    const setupComposer = () => {
        ...
        postBtn.addEventListener('click', async () => {
            const content = input.value.trim(); if (!content && !pendingImage) return;
            let imageUrl = null;
            ...
            await supabase.from('chirps').insert({ user_id: currentUser.id, username: currentUser.username, avatar_emoji: currentUser.avatar_emoji || '👤', content, image_url: imageUrl, hashtags, is_fire: isFire });
            ...
            input.value = ''; pendingImage = null; document.getElementById('pendingImageContainer').classList.add('hidden'); postBtn.disabled = true; counter.textContent = '0/280'; loadFeed();
        });
    };
    ...
    const init = async () => {
        ...
        window.addEventListener('beforeunload', () => { if (banCheckInterval) clearInterval(banCheckInterval); });
    };
    return { init };
})();
